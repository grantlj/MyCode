...
for j=1:(dn-1)
    ...
    for sitan_range=1:0.01:90
        ...
        %缝隙长度gap计算
        gap=(petalLen-n*b)/(n-1);
        
        %求出每点的横坐标
        for i=1:n
            x(i)=-ovalA+b/2+(i-1)*(gap+b);
        end
        % y是相对于平行于x轴的椭圆轴而言的
        ...
        if d<l(rm)&&d>slot(rm)&&sita(rm)>pi/2&&sita(end-1)<pi/2&&l(n)*sin(sita(n))>=max(l.*sin(sita))
            
            %首先求稳固性指标,不需要求两侧的
            SumTX=sum(-b.*(y(2:(end-1)) + x(2:(end-1))./tan(beta/2)  ) .*sin(sita(2:(end-1))).*cos(sita(2:(end-1))));
            
            %其次求加工方便指标（总切割长度）
            man=sum( l(1:(end-1)) )+sum(slot)+ovalA;
