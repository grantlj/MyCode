...
sitan= 80.1700*pi/180 ; %最外侧木条与地面的夹角
...
switch mode
    ...
    case 3
        cut(2:(end-1))=height- dia(2:(end-1));
end

%-------进行反映射，求取若需要预定的形状，求得需要在已经加长的木条上进行切去的长度
cutReal=zeros(size(dia));
cutReal(2:(end-1))=cut(2:(end-1))./sin(pi-sita(2:(end-1))-beta);
...
%     zk=[zk,zk];
    ...
    plot3( [x(end),0], [y(end),0],[0,0] ,'linewidth',8,'color','r');hold on
    plot3( [-x(end),0], [y(end),0],[0,0] ,'linewidth',8,'color','r');hold on
    
    for i=1:size(x,2)
        plot3([x(i),xk(i)],[y(i),yk(i)],[0,-zk(i)],'linewidth',8,'color',colorMap(i,:));
        hold on;
    end
